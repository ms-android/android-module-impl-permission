...
public class PermissionUtilsImpl {
    ...
    public static void requestPermission(FragmentActivity activity, PermissionCallBack callBack, String... pers) {
    ...
    private static void reqper(FragmentActivity activity) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M && activity.getApplicationInfo().targetSdkVersion > 22) {
            ...
            if (ps.length > 0) {
                ...
                if (mCallBack != null) {
                    mCallBack.onSuccess(PERMISSIONS_RUNTIME.toString());
                }
            }
        } else {
            if (mCallBack != null) {
    ...
    public static String perssionHint(List<String> deniedPermission) {
    ...
    private static void requestPermission(String... ps) {
        permissionUtil.requestPermissions(ps, new PermissionListener() {
            ...
            public void onDenied(final List<String> deniedPermission) {
                ...
                builder.setMessage("为了应用能正常运行，我们需要访问您的" + perssionHint(deniedPermission) + "权限");
            ...
            public void onShouldShowRationale(final List<String> deniedPermission) {
                ...
                AlertDialog.Builder builder = new AlertDialog.Builder(mActivity);
                builder.setTitle("权限申请失败");
                builder.setMessage("为了应用能正常运行，我们需要访问您的" + perssionHint(deniedPermission) + "权限，请前往应用详情界面，打开对应权限");
                builder.setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    @Override
                    public void onClick(DialogInterface dialog, int which) {
                ...
                builder.setNeutralButton("退出", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        if (mCallBack != null) {
                            mCallBack.onFailure(deniedPermission.toString());
                        }
                    }
                });
                builder.setCancelable(false);
